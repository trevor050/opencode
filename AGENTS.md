...
- For 20h+ unattended runs that should produce a 50-80 page deliverable, `finalHandoff` report lint/audit now defaults to `minOutlineTargetPages: 50` from `plans/operation-plan.json` `timeBudget.targetHours`. You can still pass a higher/lower explicit `minOutlineTargetPages`, but do not rely on `requireOutlineBudget` alone when reviewing older artifacts or non-final gates.
...
- Operation audits record final-handoff gate metadata under `checks.finalHandoff.gates`. For 20h+ plans, literal readiness rejects audits that do not prove `minOutlineTargetPages >= 50`, so old or weak audits must be regenerated with current code before a real 20-hour handoff can pass.
