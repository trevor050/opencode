...
- For 20h+ unattended runs that should produce a 50-80 page deliverable, pass `minOutlineTargetPages: 50` through `report_lint`, `operation_stage_gate`, and `operation_audit`; `requireOutlineBudget` alone only enforces the report against whatever target_pages already exists, even if an earlier agent undersized the outline.
...
- Literal run readiness is a full handoff proof, not just uptime: it now requires wall-clock daemon runtime/work proof, `deliverables/final/manifest.json`, and a passing `deliverables/operation-audit.json`. If you use strict long-report gates such as `minOutlineTargetPages: 50`, regenerate `operation_audit` with those gates before expecting literal readiness to pass.
