...
- Package CLI daemon lanes also set `ULMCODE_LANE_ALLOWED_TOOLS` and guarded launcher/mutation tools (`operation_recover`, `runtime_scheduler`, `runtime_daemon`, `task`, `command_supervise`, `shell`) refuse to run when absent from the lane allowlist. Keep this runtime guard; prompt-only allowlists were not enough to stop blocked lanes from recovering or relaunching work.
