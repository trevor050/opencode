...
cp -R "$PROFILE_DIR/mcp" "$TARGET_DIR/mcp"
...
chmod +x "$TARGET_DIR/mcp/pentestMCP/run-stdio.sh"
chmod +x "$TARGET_DIR/mcp/playwright-persistent/run-stdio.sh"
...
export OPENCODE_MCP_ALLOWLIST="${OPENCODE_MCP_ALLOWLIST:-websearch,agent_browser,playwright,playwright_persistent,pentestMCP,companyscope,not_human_search,openregistry}"
