...
- TUI operation display is chat-scoped too. Do not show a workspace/global active operation on the home screen; `/ulm`, `/operations`, and `/all` may list operation artifacts, but selecting one should navigate to its bound chat or create/bind a chat for legacy unbound operations.
