...
import { UlmOperations } from "./routes/ulm-operations"
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`ULMCode | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`ULMCode | ${route.data.id}`)
      return
    }

    if (route.data.type === "ulmOperations") {
      renderer.setTerminalTitle("ULMCode | ULM Operations")
    }
  })
