...
- Package CLI daemon lanes set `ULMCODE_LANE_ALLOWED_TOOLS` and in-app scheduler `taskParams` carry `allowedTools`; guarded launcher/mutation tools (`operation_recover`, `runtime_scheduler`, `runtime_daemon`, `task`, `command_supervise`, `bash`) must stay disabled when absent from a lane allowlist. Keep this runtime guard; prompt-only allowlists were not enough to stop blocked lanes from recovering or relaunching work.
